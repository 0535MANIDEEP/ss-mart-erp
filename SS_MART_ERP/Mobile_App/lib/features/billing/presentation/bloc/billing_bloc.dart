...
class BillingBloc extends Bloc<BillingEvent, BillingState> {
  ...
  final RateEngine _rateEngine;
  final DiscountEngine _discountEngine;
  final SchemeEngine _schemeEngine;
  ...
  BillingBloc({
    ...
    required RateEngine rateEngine,
    required DiscountEngine discountEngine,
    required SchemeEngine schemeEngine,
    ...
    on<SelectRateType>(_onSelectRateType);
  }
  ...
  Future<void> _onProcessPayment(
    ...
    // Auto-calculate bill-level discounts before processing payment
    BillingReady readyState = currentState;
    try {
      final billItems = currentState.items.map((item) => BillItemInput(
        productId: item.productId,
        unitPrice: item.unitPrice,
        quantity: item.quantity,
        totalAmount: item.subtotal - item.discountAmount,
      )).toList();

      final discountResult = await _discountEngine.calculateBillDiscounts(
      ...
      if (discountResult.totalDiscount > 0) {
        final preRoundTotal = currentState.subtotal + currentState.taxAmount
            - currentState.discountAmount - discountResult.totalDiscount;
        final roundedTotal = preRoundTotal.round();
        final roundOffVal = roundedTotal - preRoundTotal;

        readyState = currentState.copyWith(
          billDiscountPercent: currentState.subtotal > 0
              ? (discountResult.totalDiscount / currentState.subtotal * 100)
              : 0,
          billDiscountAmount: discountResult.totalDiscount,
      ...
      // Discount engine failure is non-fatal; proceed without auto-discount.
    }
    ...
    switch (event.paymentMode) {
      ...
      case 'UPI':
      case 'CARD':
        paidAmount = readyState.totalAmount;
        dueAmount = 0;
        break;
      case 'CREDIT':
        paidAmount = event.paidAmount;
        dueAmount = readyState.totalAmount - event.paidAmount;
        break;
      default:
        paidAmount = readyState.totalAmount;
        dueAmount = 0;
    ...
    final params = CreateBillParams(
      customerId: readyState.customerId,
      customerName: readyState.customerName,
      subtotal: readyState.subtotal,
      taxAmount: readyState.taxAmount,
      discountAmount: readyState.discountAmount + readyState.billDiscountAmount,
      roundOff: readyState.roundOff,
      totalAmount: readyState.totalAmount,
  ...
  void _onSelectRateType(SelectRateType event, Emitter<BillingState> emit) {
    final currentState = state;
    if (currentState is BillingReady) {
      emit(currentState.copyWith(rateType: event.rateType));
    }
  }
