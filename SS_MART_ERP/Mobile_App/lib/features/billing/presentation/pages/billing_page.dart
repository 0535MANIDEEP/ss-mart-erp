...
class BillingView extends StatelessWidget {
  ...
  Widget _buildCustomerBar(BuildContext context) {
    return BlocBuilder<BillingBloc, BillingState>(
      builder: (context, state) {
        ...
        final currentRateType = state is BillingReady ? state.rateType : 'selling';
            ...
            children: [
              ...
              SizedBox(
                ...
                child: ListView(
                  ...
                  children: [
                    _buildRateChip(context, 'selling', 'Retail', currentRateType),
                    _buildRateChip(context, 'rateA', 'Rate A', currentRateType),
                    _buildRateChip(context, 'rateB', 'Rate B', currentRateType),
                    _buildRateChip(context, 'rateC', 'Rate C', currentRateType),
                    _buildRateChip(context, 'wholesale', 'Wholesale', currentRateType),
                  ],
                ),
              ),
            ],
  ...
  Widget _buildRateChip(BuildContext context, String rateType, String label, String currentRateType) {
    final isSelected = rateType == currentRateType;
    return Padding(
      ...
      child: ChoiceChip(
        ...
        onSelected: (_) {
          context.read<BillingBloc>().add(SelectRateType(rateType: rateType));
        },
  ...
  Widget _buildCartItem(BuildContext context, BillItem item) {
    return BlocBuilder<BillingBloc, BillingState>(
      builder: (context, state) {
        final schemeResult = state is BillingReady ? state.schemeResults[item.productId] : null;
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        if (schemeResult != null && schemeResult.hasScheme)
                          Container(
                            ...
                            child: Text(
                              schemeResult.schemeName ?? schemeResult.schemeType,
                              style: TextStyle(
        ...
        children: [
          ...
          if (state.billDiscountAmount > 0) ...[
            ...
            if (state.discountResult != null)
              for (final d in state.discountResult!.appliedDiscounts)
                Padding(
                  padding: const EdgeInsets.only(left: 16),
                  child: _buildSummaryRow('  ${d.ruleName}', '-\u20B9${d.amount}', isSmall: true),
                ),
          ],
          if (state.roundOff != 0)
  ...
  Widget _buildSummaryRow(String label, String value, {bool isBold = false, bool isLabel = false, bool isSmall = false}) {
    return Padding(
      padding: EdgeInsets.symmetric(vertical: isSmall ? 0 : 2),
      child: isLabel
          ? Text(label, style: TextStyle(fontSize: 12, color: Colors.grey[600]))
          : Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text(label, style: TextStyle(fontSize: isBold ? 17 : isSmall ? 11 : 14, fontWeight: isBold ? FontWeight.bold : FontWeight.normal, color: isSmall ? Colors.grey[600] : null)),
                Text(value, style: TextStyle(fontSize: isBold ? 17 : isSmall ? 11 : 14, fontWeight: isBold ? FontWeight.bold : FontWeight.normal, color: isSmall ? Colors.grey[600] : null)),
              ],
            ),
    );
  }
